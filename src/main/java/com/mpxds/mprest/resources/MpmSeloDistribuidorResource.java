...
@RestController
@RequestMapping(value="/mSeloDistribuidors")
public class MpmSeloDistribuidorResource {
	//
	@Autowired
	private MpmSeloDistribuidorService mpService;
	
	@RequestMapping(value="/{id}", method=RequestMethod.GET)
	public ResponseEntity<?> findId(@PathVariable Integer id) {
		//
		MpmSeloDistribuidor mpObj = mpService.buscarId(id);
		//
